...
class FairPlay {
    ...
    /**
     * fp-setup v2 phase 1. Only mode 2 can be answered correctly; everything else is refused.
     *
     * Answering modes 0, 1 and 3 with mode 2's material used to "work" in the sense that the
     * handshake completed — and that was the problem. The sender accepted the reply, wrapped the
     * audio key against cryptographic material we had guessed, and handed back a key that unwrapped
     * to a plausible-looking 16 bytes that was simply wrong. The result was a session that reached
     * RECORD, showed metadata, accepted transport commands, and played noise: roughly 5 of every 24
     * ALAC frames decoding before the decode-health gate muted the stream.
     *
     * Failing here instead is worse for the sender and better for the user. macOS Music reports that
     * it cannot connect, which is true, rather than appearing to play a track that is silent or
     * garbled. A receiver that says "no" is diagnosable; one that says "yes" and then produces noise
     * sends people looking at their speakers, their network and their audio delay setting.
     *
     * Lifting this needs the real mode-0/1/3 tables, which exist in no public implementation and
     * must be captured from Apple hardware. See the README.
     */
    private fun setupV2(mode: Int): ByteArray {
        if (mode != V2_KNOWN_MODE) {
            throw IllegalArgumentException(
                "FairPlay v2 mode $mode is not supported — only mode $V2_KNOWN_MODE has a known " +
                    "reply table. Refusing rather than answering with the wrong key material.",
            )
        }
        return REPLY_V2.copyOf()
    }
    ...
    companion object {
        ...
        /** The only fp-setup v2 mode with a published reply table. See [setupV2]. */
        const val V2_KNOWN_MODE = 2
